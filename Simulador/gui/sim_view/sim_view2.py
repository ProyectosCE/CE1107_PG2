...
class SimView2(BaseSimView):
    ...
    def _draw_content(self):
        ...
        self.mux_ForwardAE = HardwareBlock(self.canvas, 850, 100, 60, 50, "MUX\nForwardAE",
                                        fill="lightgray", border_color="#00838F", tags="mux_ForwardAE")
        self.mux_ForwardBE = HardwareBlock(self.canvas, 850, 200, 60, 50, "MUX\nForwardBE",
                                        fill="lightgray", border_color="#00838F", tags="mux_ForwardBE")

        self.mux_srcB = HardwareBlock(self.canvas, 1000, 200, 50, 50, "MUX\nSrcB",
                                      fill="lightgray", border_color="#00838F", tags="mux_srcB")  # 770 + 30 + 50 = 850

        self.adderPC = HardwareBlock(self.canvas, 1000, 270, 60, 80, "Adder PC",
                                     fill="lightgray", border_color="#5E35B1", shape_type="trapezoid", tags="adderPC")

        self.alu = HardwareBlock(self.canvas, 1100, 120, 100, 160, "ALU",
                                 fill="lightgray", border_color="#5E35B1", shape_type="trapezoid",
                                 tags="alu")  # 850 + 50 + 50 = 950

        # ──────── 4. MEMORY ACCESS ────────

        self.reg3 = HardwareBlock(self.canvas, 1250, 60, 30, 330, "Reg3",
                                  fill="lightgray", border_color="#33691E", tags="reg3")  # 950 + 100 + 50 = 1100

        self.data_mem = HardwareBlock(self.canvas, 1330, 120, 120, 160, "Data Mem",
                                      fill="lightgray", border_color="#EF6C00", tags="data_mem")  # 1100 + 30 + 50 = 1180

        # ──────── 5. WRITE BACK ────────

        self.reg4 = HardwareBlock(self.canvas, 1500, 60, 30, 330, "Reg4",
                                  fill="lightgray", border_color="#33691E", tags="reg4")  # 1180 + 120 + 50 = 1350

        self.mux_result = HardwareBlock(self.canvas, 1570, 170, 60, 60, "MUX\nResult",
                                        fill="lightgray", border_color="#00838F",
                                        tags="mux_result")  # 1350 + 30 + 50 = 1430
        #ports:

        self.mux_ForwardAE.add_port("sel", self.mux_ForwardAE.width // 2, 0)
        self.mux_ForwardAE.add_port("A", 0, 10)
        self.mux_ForwardAE.add_port("B", 0, 25)
        self.mux_ForwardAE.add_port("C", 0, 40)
        self.mux_ForwardAE.add_port("out", self.mux_ForwardAE.width, self.mux_ForwardAE.height // 2)

        self.mux_ForwardBE.add_port("sel", self.mux_ForwardBE.width // 2, 0)
        self.mux_ForwardBE.add_port("A", 0, 10)
        self.mux_ForwardBE.add_port("B", 0, 25)
        self.mux_ForwardBE.add_port("C", 0, 40)
        self.mux_ForwardBE.add_port("out", self.mux_ForwardBE.width, self.mux_ForwardBE.height // 2)

        self.mux_pc.add_port("sel", self.mux_pc.width // 2, 0)
        ...
        self.mux_srcB.add_port("sel", self.mux_srcB.width // 2, 0)
        self.mux_srcB.add_port("A", 0, 10)
        self.mux_srcB.add_port("B", 0, 30)
        ...
        self.mux_result.add_port("sel", self.mux_result.width // 2, 0)
        ...
        # reg1 → MUX ForwardAE (A)
        Connection(self.canvas, self.reg1, self.mux_ForwardAE,
                   source_port="out", target_port="A", arrow=False).draw()

        # MUX ForwardAE → ALU (opA)
        Connection(self.canvas, self.mux_ForwardAE, self.alu,
                   source_port="out", target_port="opA", arrow=False).draw()

        # reg1 → MUX ForwardBE (A)
        Connection(self.canvas, self.reg1, self.mux_ForwardBE,
                   source_port="out", target_port="A", arrow=False).draw()
        # MUX ForwardBE → MUX SrcB (A)
        Connection(self.canvas, self.mux_ForwardBE, self.mux_srcB,
                   source_port="out", target_port="A", arrow=False).draw()
        ...
        for unit in [self.mux_pc, self.reg_pc, self.instruction_mem, self.adder,
                     ...
                     self.mux_srcB, self.adderPC, self.mux_ForwardAE, self.mux_ForwardBE, self.alu, self.reg3,
        ...
        self.units = {
            ...
            "mux_ForwardAE": self.mux_ForwardAE,
            "mux_ForwardBE": self.mux_ForwardBE,
            "alu": self.alu,
            "reg3": self.reg3,
